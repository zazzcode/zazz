...
### Option A: GitButler-style virtual branches in one working directory

Summary:
Use a single working tree and track branch ownership through a virtualized branch layer, with stacked branches and PR base updates managed inside that local model.

Pros:

- familiar to teams already using GitButler
- official stacked-branch and branch-moving workflows already exist in that model
- can offer a polished branch-management experience
- avoids multiple checked-out directories

Cons:

- less natural for multiple agents
- conflict resolution stays trapped in a virtualized model
- harder to run multiple isolated test and dev loops in parallel
- network-style dependency graphs are not a natural fit
- the local orchestration model is not built around one-agent-per-worktree isolation

### Option B: Worktree-native linear stack manager

Summary:
Use one real branch and one real worktree per stack layer. Optimize the whole product around sequential parent-child chains.

Pros:

- simplest worktree-native path to value
- maps cleanly onto GitHub stacked PR flows
- ideal first CLI product for both humans and agents
- lower implementation risk than full DAG-first orchestration

Cons:

- less flexible than a graph model
- downstream work still remains ordered by stack shape
- some branches may wait longer for stable parent changes
- risks baking linear assumptions into metadata and CLI semantics that later become painful to undo

### Option C: Worktree-native DAG orchestrator

Summary:
Use explicit metadata to support one parent with multiple children and eventual convergence nodes.

Pros:

- best fit for multi-agent parallelism
- can model real dependency graphs instead of only chains
- offers a product capability that existing stacked-PR tools do not handle well

Cons:

- much harder GitHub lifecycle semantics
- more complex waiting, merge, and rebase orchestration
- convergence nodes can require richer conflict policy and staging logic

### Option D: GitHub-native stacked PR workflow with minimal local orchestration

Summary:
Lean on GitHub conventions and keep local worktree management mostly manual.

Pros:

- smaller product scope
- can reach value quickly for stacked PR creation

Cons:

- weak local orchestration story
- less differentiated for agents
- limited control over conflict routing and migration workflows
...
- provides the core primitive we want: multiple linked working trees with shared Git object storage
- already supports worktree lifecycle operations such as add, move, lock, list, repair, and prune
- keeps the underlying implementation Git-native and portable

What it does not do:

- no graph model for branch relationships
- no PR awareness
- no orchestration around rebases, propagation, or review
- no agent ownership or conflict-routing model

Takeaway:

- native Git worktree is the substrate, not the finished product
...
- polished desktop UX
- virtual branches let one user work on multiple independent or dependent branches inside one directory
- stacked branches support parent-relative PR targeting and local stack orchestration
- branch movement and stack reshaping are already user-friendly
- upstream integration and stack maintenance are handled locally in the client rather than by a hosted cloud control plane
- GitButler's own engineering writing shows a very strong understanding of the exact pain this product is also addressing: multiple branches or topics active at the same time, with less stashing, switching, and branch friction

What GitButler is explicitly optimized for:

- several branches applied to a single working directory at the same time
- drag-and-drop reassignment of changes between lanes and stacks
- fast, ergonomic branch manipulation for one human user in one workspace

What matters for this proposal:

- GitButler's core model is powerful precisely because everything is happening in one workspace
- that same property is also the biggest architectural difference from this proposal
- GitButler can be excellent for one human juggling multiple changesets
- it is a less natural fit for multiple agents that each need stable filesystem isolation, separate terminals, separate test loops, and independent local execution context
- GitButler's own docs also make clear that its virtual branch model is not meant to be mixed casually with normal Git branching commands in the same workflow, which is an important constraint for teams that want a more Git-native and automation-friendly execution model
- GitButler's own worktree comparison is also honest about the tradeoff: worktrees provide separate working directories and staging areas per branch, but that comes with more friction and more visible divergence risk

What it does better than this proposal today:

- much more mature end-user workflow and interface
- easier immediate adoption for a single human who wants GUI-first branch manipulation
- virtual branch lanes and drag-and-drop change routing are powerful for one-person workflows
- because virtual branches start from one workspace view, GitButler can avoid some categories of local divergence that worktree-based systems must manage explicitly
- GitButler is probably better if the main job is "help one human juggle multiple PRs and related changes with as little friction as possible"

What this proposal aims to do better:

- one real worktree per branch for true agent isolation
- independent terminals, tests, and running environments per branch
- DAG-native core model rather than only a polished sequential stack experience
- explicit orchestration for multi-agent execution and semantic migration
- a more natural operational model for "one agent, one worktree, one branch"
- cleaner fit with standard Git and worktree-based automation
- a better answer to the specific problem "how do I let multiple agents continue in parallel while a human reviewer is behind, then automatically propagate upstream changes across already-open downstream PRs"

Where this proposal may be worse:

- multiple worktrees cost more local disk and setup complexity
- a worktree-native UX is less magically convenient than a single-directory virtual-branch UI
- it may take longer to match GitButler's day-one product polish
- conflict management may be harder because real isolation exposes real downstream divergence that GitButler can sometimes smooth over inside one workspace
- if the automation is not excellent, this product can feel heavier and more cumbersome than GitButler for solo-human workflows
- worktrees also have concrete ergonomic costs that GitButler has called out itself: extra editor windows, duplicate ignored/build artifacts, and easier accidental divergence between parallel branches

Takeaway:

- GitButler is the clearest direct comparison and the strongest competitor on human UX polish
- this product should differentiate on worktree-native execution, agent workflows, Git-native automation, and DAG-aware orchestration rather than trying to be a visual clone
- if the main user is a single human doing stacked branches in one workspace, GitButler may simply be the better product today
- if the main user is one human coordinating multiple agents that need true concurrent isolation, this proposal has a stronger architectural fit

### Honest Comparison Summary: GitButler vs This Proposal

The two products overlap, but they are not optimized for the same center of gravity.

GitButler appears optimized for:

- one human user
- one workspace
- minimal branch friction
- smooth visual branch manipulation

This proposal is optimized for:

- one human coordinating multiple agents
- many isolated worktrees
- branch and PR dependency orchestration
- automated downstream propagation after upstream review and merge events

Brutally honest conclusion:

- if you adopt GitButler, you may still have the exact problem that motivated your move to worktrees in the first place: no true per-agent filesystem isolation
- if you build this proposal, you may solve that isolation problem, but you will inherit a lot more complexity than GitButler has to manage
- the project is worthwhile only if the agent-isolation and automated-propagation benefits are important enough to justify that additional complexity

### Graphite

What it does well:

- strong GitHub stacked-PR workflow
- automation around creating, submitting, and syncing stacked branches
- good review guidance and operational maturity around stacked diffs

What it does better than this proposal today:

- more mature GitHub-centered stacked PR workflow
- likely better immediate fit for teams that only need sequential stacks and review acceleration

What this proposal aims to do better:

- local worktree orchestration instead of branch-stack workflow only
- explicit support for agents owning isolated execution environments
- semantic conflict escalation and migration
- DAG-capable local graph, not just branch stacks as a developer productivity layer

Where this proposal may be worse:

- more operational complexity
- harder MVP path because the local orchestration surface is much larger

Takeaway:

- Graphite is a strong adjacent competitor for stacked PRs, but not the same product category if we fully commit to worktree-native and agent-native execution

### Open-source stacked-PR CLIs

Examples found during research:

- `ghstack`, which submits stacks of diffs to GitHub as separate pull requests
- `git-stack`, which manages stacked branches and PR-oriented stacked-diff workflows

What they do well:

- stacked PR creation and update workflows already exist in open source
- they validate that small, dependent PR chains are a real and useful workflow
- they reduce manual PR bookkeeping

Where they stop short of this proposal:

- they are primarily stacked-PR tools, not worktree-native orchestration systems
- they do not appear to center the product around one-agent-per-worktree execution
- they do not appear to combine local worktree DAG management, desktop graph visualization, and AI-assisted downstream conflict propagation into one system

Takeaway:

- the stacked-PR part of the idea is not unique
- the worktree-native, agent-native, propagation-heavy combination still looks differentiated

### Open-source worktree and agent workflow tools

Examples found during research:

- `emdash`, an orchestration layer for running multiple coding agents in parallel in isolated Git worktrees
- `git-worktree-runner`, a CLI that automates worktree creation, editor integration, AI tool launching, and setup hooks
- `gwq`, which focuses on ergonomic worktree management for concurrent development and parallel AI coding workflows

What they do well:

- they validate that the market is moving toward worktrees as the right isolation primitive for AI agents
- they smooth out the rough edges of worktree creation, environment copying, and parallel-session management
- some of them already include editor or AI-tool integration

Where they stop short of this proposal:

- they are mostly about worktree/session management, not PR dependency orchestration
- they do not appear to offer the branch-graph plus per-branch PR propagation model you are targeting
- they do not appear to center human review checkpoints and downstream PR refresh as the core workflow

Takeaway:

- there is definitely real demand for agent-oriented worktree tooling
- your proposal sits one layer above these tools by adding review-aware branch propagation and DAG orchestration

### Existing UI and visualization overlap

Examples found during research:

- `Graft`, which brings stacked branch workflows and worktree management into Visual Studio
- editor extensions such as `git-worktree-cursor` that visualize and manage worktrees inside an editor

What they do well:

- they validate the usefulness of visual stack/worktree management
- they show that users do want a control-center style interface once multiple worktrees exist

Where they stop short of this proposal:

- they do not appear to be centered on local DAG propagation across already-open PRs
- they do not appear to make AI-assisted downstream conflict resolution the core differentiator

Takeaway:

- the UI idea is not outlandish
- the challenge is not whether a graph UI is useful, but whether the underlying orchestration is strong enough to deserve one

### Sapling

What it does well:

- first-class stack manipulation and history editing
- strong conceptual model for stacked changes

What it does better than this proposal today:

- stack-aware source control is part of the core system rather than layered on top

What this proposal aims to do differently:

- stay Git-native and GitHub-compatible
- avoid asking teams to adopt a different source control toolchain
- focus on local worktrees, PR orchestration, and human-plus-agent workflows

Where this proposal may be worse:

- layering on top of Git means inheriting Git's rough edges
- some stack-native operations may never feel as elegant as they do in a tool designed around stacks from first principles

Takeaway:

- Sapling validates the value of stack-native workflows, but this proposal deliberately chooses Git compatibility and worktree-native execution over replacing the underlying SCM model
...
- all managed worktrees for an active graph live on the same machine
- the CLI, desktop app, daemon, and agents all have local filesystem access to the same repo container
- the DAG state is therefore tracked locally and does not require a distributed coordination layer in the first release

### Repo-local orchestration state

The canonical DAG and worktree state should live with the repo container, not only in the user's home directory.

Recommended location:

- `repo-container/.zazz-wt/`

Recommended contents:

- `graph.json` or `graph.db` for branch and dependency relationships
- `worktrees.json` for worktree registry and status
- `prs.json` or table-backed PR state
- `events/` or event tables for sync and migration history
- `locks/` for local operational locking if needed

Reasoning:

- the graph belongs to the repo, not to the machine globally
- repo-local state is easier to inspect, back up, repair, and reason about
- the container root is the natural place because it already holds `.bare/` and the managed sibling worktrees
- single-machine local access means the product can rely on filesystem-local state rather than designing for distributed consensus or remote worktree coordination

### User-level application state

The user home directory should still hold machine-local application state.

Recommended locations:

- Linux config: `~/.config/<product-name>/`
- Linux state/cache: `~/.local/state/<product-name>/` and `~/.cache/<product-name>/`
- macOS app support: `~/Library/Application Support/<product-name>/`
- macOS caches/logs: `~/Library/Caches/<product-name>/`
...
- keep canonical graph and worktree state repo-local in `.zazz-wt/`
- keep machine-local preferences, credentials, caches, and logs under `~/.zazz/`
- keep product-specific global settings under subdirectories such as `~/.zazz/worktrees/` and `~/.zazz/board/`
...
Recommended repo-local state layout:
...
- keep per-repo DAG state, worktree registry, refresh history, and conflict artifacts in `.zazz-wt/`
- keep machine-global settings, model/provider credentials, caches, and logs in `~/.zazz/`
- keep worktrees-specific global settings in `~/.zazz/worktrees/`
- do not make `~/.zazz/` the canonical source of per-repo graph truth
